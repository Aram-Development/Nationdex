...
class AramPacks(commands.Cog):
    ...
    async def initialize_promocodes(self):
        ...
        try:
            ...
            log.info(f"AramPacks system initialized with {len(active_promocodes)} active codes out of {len(all_promocodes)} total")
            ...
            log.info(f"AramPacks system initialized with errors. Some features may not work correctly.")
    ...
    async def process_promocode(self, interaction: discord.Interaction, code: str):
        ...
        try:
            ...
            await interaction.response.send_message("❌ Database error occurred. Please try again later.", ephemeral=True)
            return
        ...
        try:
            ...
            if not rewards:
                await interaction.response.send_message("❌ This promocode has no rewards configured.", ephemeral=True)
                return
        except Exception as e:
            log.exception(f"Error getting promocode rewards for {code}: {e}")
            await interaction.response.send_message("❌ Error retrieving promocode rewards.", ephemeral=True)
            return
        ...
        try:
            ...
            if specific_ball_id:
                ...
                if specific_ball_id not in balls:
                    log.error(f"Promocode {code} references non-existent ball ID: {specific_ball_id}")
                    await interaction.response.send_message("❌ This promocode is misconfigured. Please contact support.", ephemeral=True)
                    return
                ...
                if not enabled_balls:
                    await interaction.response.send_message("❌ No collectibles available for rewards.", ephemeral=True)
                    return
            ...
            await interaction.response.send_message("❌ Error processing rewards. Please try again later.", ephemeral=True)
            return
